...
namespace ForbiddenClient.FoldersManager
{
    class Create
    {
        ...
        public static void Initialize()
        {
            ...
            string url = "https://urmomisprettygayngluwurawrxdddsdasdwadawjhdrawrhehe.glitch.me/SDK/";

            if (Directory.Exists("Joanpixer"))
            {
                Directory.Move("Joanpixer", "Forbidden");
            }
            else if (!Directory.Exists("Forbidden"))
            {
            ...
            {
                client.Headers.Add("user-agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36)");
                client.DownloadFile(new Uri($"{url}Console.png"), "Forbidden\\Console.png");
            }

            if (!File.Exists(Environment.CurrentDirectory + "\\Forbidden\\Welcome.wav"))
            {
                client.Headers.Add("user-agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36)");
            ...
            {
                client.Headers.Add("user-agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36)");
                client.DownloadFile(new Uri($"{url}sound.wav"), "Forbidden\\sound.wav");
            }

            if (!Directory.Exists("Forbidden\\VRCA"))
            {
                Directory.CreateDirectory("Forbidden\\VRCA");
            }

            if (!File.Exists("Forbidden\\Config.ini"))
            {
                File.Create("Forbidden\\Config.ini");
                Ini = new IniFile("Forbidden\\Config.ini");
            ...
            if (!File.Exists("Forbidden\\AvatarFavorites.json"))
            {
