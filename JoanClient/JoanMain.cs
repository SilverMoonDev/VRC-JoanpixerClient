    ...
    public class JoanpixerMain : MelonMod
    {
        ...
        public override void OnApplicationStart()
        {
            ...
            foreach (var eventListener in eventListeners)
            {
                ...
                {
                    JoanpixerClient.JoanpixerMain.Logger.Error("Encountered an exception while running OnApplicationStart of \"" + eventListener.GetType().FullName + "\":\n" + ex);
                }
            }
        ...
        public override void OnSceneWasUnloaded(int buildIndex, string sceneName)
        {
            foreach (var eventListener in eventListeners)
            {
                ...
                {
                    JoanpixerClient.JoanpixerMain.Logger.Error("Encountered an exception while running OnSceneUnload of \"" + eventListener.GetType().FullName + "\":\n" + ex);
                }
            }
        }
        ...
        public override void OnSceneWasInitialized(int buildIndex, string sceneName)
        {
            foreach (var eventListener in eventListeners)
            {
                ...
                {
                    JoanpixerClient.JoanpixerMain.Logger.Error("Encountered an exception while running OnSceneInit of \"" + eventListener.GetType().FullName + "\":\n" + ex);
                }
            }
        }
        ...
        public override void OnUpdate()
        {
            if (RoomManager.field_Internal_Static_ApiWorldInstance_0?.type == VRC.Core.InstanceAccessType.InvitePlus && Input.GetKeyUp(KeyCode.K) || RoomManager.field_Internal_Static_ApiWorldInstance_0?.type == VRC.Core.InstanceAccessType.InviteOnly && Input.GetKeyUp(KeyCode.K))
            {
                ...
                for (int i = 0; i < uwu; i++)
                {
                    ...
                    {
                        JoanpixerClient.JoanpixerMain.Logger.Msg($"{i}: {Utils.CurrentUser.gameObject.GetComponentInChildren<SkinnedMeshRenderer>().sharedMesh.GetBlendShapeName(i)}: {Utils.CurrentUser.gameObject.GetComponentInChildren<SkinnedMeshRenderer>().GetBlendShapeWeight(i)}");
                    }
                }
