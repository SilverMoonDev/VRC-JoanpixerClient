        ...
        public override void OnUpdate()
        {
            if (RoomManager.field_Internal_Static_ApiWorldInstance_0?.type == VRC.Core.InstanceAccessType.InvitePlus && Input.GetKeyUp(KeyCode.K) || RoomManager.field_Internal_Static_ApiWorldInstance_0?.type == VRC.Core.InstanceAccessType.InviteOnly && Input.GetKeyUp(KeyCode.K))
            {
                ...
                for (int i = 0; i < uwu; i++)
                {
                    ...
                    {
                        MelonLogger.Msg($"{i}: {Utils.CurrentUser.gameObject.GetComponentInChildren<SkinnedMeshRenderer>().sharedMesh.GetBlendShapeName(i)}: {Utils.CurrentUser.gameObject.GetComponentInChildren<SkinnedMeshRenderer>().GetBlendShapeWeight(i)}");
                    }
                }
            ...
            if (Time.time > OnUpdateRoutineDelay && VRCUtils.IsWorldLoaded)
            {
                ...
                if (Murder4.worldLoaded && Murder4.CluesESP && PatchManager.InMurder4Game)
                {
