    ...
    internal static class PatchManager
    {
        ...
        private static bool OnEvent(ref EventData __0)
        {
            ...
            try
            {
                if (__0 != null)
                {
                    ...
                    if (obj != null)
                    {
                        //Utils.ConsoleLog(Utils.ConsoleLogType.Msg, __0.Code.ToString());
                        switch (__0.Code)
                        {
                            case 33:
        ...
        private static IEnumerator Delay()
        {
            ...
            /*if (!loggedin)
            {
                new Thread(() =>
                {
                    System.Media.SoundPlayer player = new(Environment.CurrentDirectory + "\\Forbidden\\Welcome.wav");
                    player.PlaySync();
                }).Start();
            }*/
            loggedin = true;
            ...
            try
            {
                ...
                else if (Prison.worldLoaded)
                {
                    ...
                    foreach (var data in Resources.FindObjectsOfTypeAll<GameObject>())
                    {
                    ...
                    foreach (var scorecard in Resources.FindObjectsOfTypeAll<GameObject>().Where(scorecard => scorecard.gameObject.name.StartsWith("Scorecard")))
                    {
                        Prison.scorecards.Add(scorecard);
                    }
                    foreach (var nametag in Resources.FindObjectsOfTypeAll<TextMeshProUGUI>().Where(nametag => nametag.gameObject.transform.parent.gameObject.name.StartsWith("Nametag")))
                    {
                        Prison.nametags.Add(nametag);
                    }
                    foreach (var patreontext in Resources.FindObjectsOfTypeAll<TextMeshProUGUI>().Where(patreontext => patreontext.gameObject.name.StartsWith("Text Tier")))
                    {
                        Prison.patreons.Add(patreontext);
                    }
                    foreach (var crate in Resources.FindObjectsOfTypeAll<GameObject>().Where(crate => crate.gameObject.name.Equals("Crate")))
                    {
                ...
                {
                    var bclub = (Environment.CurrentDirectory + "\\Forbidden\\Resources\\JustBClub.png").LoadSpriteFromDisk();
