...
namespace ForbiddenClient
{
    ...
    {
        internal static PedalOption Speedhack;
        internal static PedalOption Noclip;
        internal static PedalOption Headlight;
        ...
        internal static void Start()
        {
            ...
            #endregion
            VRCActionMenuPage.AddSubMenu(ActionMenuPage.Main, "Forbidden", () =>
            {
                CustomSubMenu.AddButton("Restart & Rejoin", () =>
                {
                    API.Popups.ShowConfirmPopup("Restart", "Are you sure that you want to Restart?",
                        "Yes", () =>
                        {
                            Utils.RestartAndRejoin();
                        },
                        "No",
                ...
                CustomSubMenu.AddSubMenu("Player", () =>
                {
                    Headlight = CustomSubMenu.AddToggle("Headlight", Utils.LightState, (val) =>
                    {
                        ...
                        MenuUI.Headlight.SetToggleState(Utils.LightState, false);
                        if (Utils.LightState)
                        {
                            Headlight.TogglePedal(true);
                        }
                        else
                        {
                            Headlight.TogglePedal(false);
                        }
                    });
                    CustomSubMenu.AddSubMenu("Movement", () =>
                    {
                        
                        Noclip = CustomSubMenu.AddToggle("Noclip", MenuUI.NoclipOn, (val) =>
                        {
                            if (MenuUI.NoclipOn)
                            {
                                MenuUI.Noclip.SetToggleState(false, true);
                            ...
                            else
                            {
                                ...
                                MenuUI.Noclip.SetToggleState(true, true);
                                MenuUI.NoclipOn = true;
                            }
                        });
                        Speedhack = CustomSubMenu.AddToggle("Speedhack", MenuUI.SpeedOn, (val) =>
                        {
                            if (MenuUI.SpeedOn)
                            {
                                MenuUI.Speedhack.SetToggleState(false, false);
                                MenuUI.SpeedOn = false;
                                Features.Speedhack.speedEnabled = false;
                            }
                            else
                            {
                                MenuUI.Speedhack.SetToggleState(true, false);
                                MenuUI.SpeedOn = true;
                ...
                {
                    CustomSubMenu.AddSubMenu("Murder 4", () =>
                    {
                        CustomSubMenu.AddToggle("Auto Start", Murder4.autostart, (val) =>
                        {
                            if (!Murder4.worldLoaded) return;
                            Murder4.autostart = val;
                            MelonLoader.MelonCoroutines.Start(Murder4.AutoStart());
                        });
                        CustomSubMenu.AddButton("Start", () =>
                        {
                            if (!Murder4.worldLoaded) return;
                            Murder4.CallGameLogic("SyncStart");
                        });
                        CustomSubMenu.AddToggle("PermaLock Door", Murder4.permalock, (val) =>
                        {
                            if (!Murder4.worldLoaded) return;
                            Murder4.permalock = val;
                            if (val)
                            {
                                Murder4.permalockeddoor = null;
                                Murder4.checkdoor = true;
                            }
                            else
                            {
                                Murder4.permalockeddoor = null;
                                Murder4.checkdoor = false;
                            }
                        });
                        CustomSubMenu.AddSubMenu("Change Role", () =>
                        {
                            if (!Murder4.worldLoaded) return;
                            CustomSubMenu.AddButton("Bystander", () =>
                            {
                                Utils.SetRole(Utils.CurrentUser.prop_Player_0, "Bystander");
                            });
                            CustomSubMenu.AddButton("Detective", () =>
                            {
                                Utils.SetRole(Utils.CurrentUser.prop_Player_0, "Detective");
                            });
                            CustomSubMenu.AddButton("Murderer", () =>
                            {
                ...
                {
                    CustomSubMenu.AddSubMenu("AmongUs", () =>
                    {
                        CustomSubMenu.AddToggle("Auto Start", AmongUs.autostart, (val) =>
                        {
                            if (!AmongUs.worldLoaded) return;
                            AmongUs.autostart = val;
                            MelonLoader.MelonCoroutines.Start(AmongUs.AutoStart());
                        });
                        CustomSubMenu.AddButton("Start", () =>
                        {
                            if (!AmongUs.worldLoaded) return;
                        ...
                        {
                            if (!AmongUs.worldLoaded) return;
                            CustomSubMenu.AddButton("Crewmate", () =>
                            {
                                Utils.SetRole(Utils.CurrentUser.prop_Player_0, "Bystander");
                            });
                            CustomSubMenu.AddButton("Imposter", () =>
                            {
                                Utils.SetRole(Utils.CurrentUser.prop_Player_0, "Murderer");
                            });
                        });
                        CustomSubMenu.AddSubMenu("Sabotages", () =>
                        {
                            if (!AmongUs.worldLoaded) return;
                            CustomSubMenu.AddButton("Sabotage All Doors", () =>
                            {
                                if (!AmongUs.worldLoaded) return;
                            ...
                            CustomSubMenu.AddButton("Sabotage All", () =>
                            {
                                if (!AmongUs.worldLoaded) return;
                            ...
                            CustomSubMenu.AddButton("Sabotage Lights", () =>
                            {
                                if (!AmongUs.worldLoaded) return;
                                AmongUs.CallGameLogic("SyncDoSabotageLights");
                            });
                            CustomSubMenu.AddButton("Sabotage Comms", () =>
                            {
                                if (!AmongUs.worldLoaded) return;
                                AmongUs.CallGameLogic("SyncDoSabotageComms");
                            });
                            CustomSubMenu.AddButton("Sabotage Reactor", () =>
                            {
                                if (!AmongUs.worldLoaded) return;
                                AmongUs.CallGameLogic("SyncDoSabotageReactor");
                            });
                            CustomSubMenu.AddButton("Sabotage Oxygen", () =>
                            {
                                if (!AmongUs.worldLoaded) return;
                                AmongUs.CallGameLogic("SyncDoSabotageOxygen");
                            });
                            CustomSubMenu.AddButton("Repair All", () =>
                            {
                                if (!AmongUs.worldLoaded) return;
                ...
                {
                    CustomSubMenu.AddSubMenu("Prison Escape", () =>
                    {
                        CustomSubMenu.AddToggle("Auto Start", Prison.autostart, (val) =>
                        {
                            if (!Prison.worldLoaded) return;
                            Prison.autostart = val;
                            MelonLoader.MelonCoroutines.Start(Prison.AutoStart());
                        });
                        CustomSubMenu.AddButton("Start", () =>
                        {
                            if (!Prison.worldLoaded) return;
                        ...
                        {
                            CustomSubMenu.AddButton("Get Keycard", () =>
                            {
                                if (!Prison.worldLoaded) return;
                                Prison.gamemanager.SendCustomEvent("_TakeKeycard");
                            });
                            CustomSubMenu.AddButton("Respawn", () =>
                            {
                                if (!Prison.worldLoaded) return;
                                Prison.gamemanager.SendCustomEvent("_SpawnPlayer");
                            });
                            CustomSubMenu.AddToggle("Get Money Loop", Prison.loop, (val) =>
                            {
                                if (!Prison.worldLoaded) return;
                                Prison.loop = val;
