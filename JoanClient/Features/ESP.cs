...
namespace JoanpixerClient.Features
{
    internal class HighlightsComponent
    {
        ...
        private static HighlightsFXStandalone GetHighlightsFX(APIUser apiUser)
        {
            ...
            {
                if (APIUser.IsFriendsWith(apiUser.id) && Worlds.Murder4.worldLoaded && Worlds.Murder4.MurderText.GetComponent<Text>().text.Contains(apiUser.displayName))
                    return _murdererHighlights;
                if (Worlds.Murder4.worldLoaded && Worlds.Murder4.MurderText.GetComponent<Text>().text.Contains(apiUser.displayName) && !APIUser.IsFriendsWith(apiUser.id))
                    return _murdererHighlights;
            }
